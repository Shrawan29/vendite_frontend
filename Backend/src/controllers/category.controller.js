...
const createCategory = asyncHandler(async (req, res) => {
    ...
    // Validate that name and description exist and are not empty after trimming
    ...
    const existedCategory = await Category.findOne({ name: trimmedName });
...
const updateCategory = asyncHandler(async (req, res) => {
    ...
    const updatedCategory = await Category.findByIdAndUpdate(id, { name: name, description: description, parentCategory: parentCategory || null }, { new: true });

    if (!updatedCategory) {
        return res.status(404).json(apiResponse(404, "Category not found"));
    }

    return res.status(200).json(apiResponse(200, "Category updated successfully", updatedCategory));
})
